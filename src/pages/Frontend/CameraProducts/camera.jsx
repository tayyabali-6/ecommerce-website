...
const Camera = () => {
  ...
  const fetchCamera = async () => {
    try {
      ...
      if (data.success) {
        // Filter products with category "camera"
        const cameraOnly = data.products.filter((item) => 
          item.category.toLowerCase() === "camera"
        );
        setProducts(cameraOnly);
      } else {
  ...
  return (
    <div className="container py-5">
      <div className="d-flex align-items-center mb-4">
        ...
        <h4 className="text-danger m-0">Camera Products</h4>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-center text-muted">No camera products found.</p>
      ) : (
