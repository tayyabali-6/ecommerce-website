...
const Tech = () => {
  ...
  const fetchTechProducts = async () => {
    try {
      ...
      if (data.success) {
        // Filter products with category "tech"
        const techOnly = data.products.filter((item) =>
          item.category && item.category.toLowerCase() === "tech"
        );
        setProducts(techOnly);
      } else {
  ...
  return (
    <div className="container py-5">
      <div className="d-flex align-items-center mb-4">
        ...
        <h4 className="text-danger m-0">Tech Products</h4>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-center text-muted">No tech products found.</p>
      ) : (
