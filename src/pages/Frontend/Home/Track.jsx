...
const Track = () => {
  ...
  // ✅ Now safely filter data
  const fashion = getAllProductSafe.filter(p => p.category === "fashion").slice(0, 4);
  const mobile = getAllProductSafe.filter(p => p.category === "mobile").slice(0, 4);
  const laptop = getAllProductSafe.filter(p => p.category === "laptop").slice(0, 4);
  const books = getAllProductSafe.filter(p => p.category === "books").slice(0, 4);
      ...
      <div className="row g-4">
        ...
        {data && data.length > 0 ? (
          ...
          <div className="col-12 text-center py-4">
            <p className="text-muted">No products found in {title} category</p>
          </div>
        )}
      </div>
    </>
